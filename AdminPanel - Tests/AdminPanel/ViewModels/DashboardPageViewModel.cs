...
namespace AdminPanel.ViewModels
{
    public class DashboardPageViewModel : ViewModelBase
    {
        ...
        public DashboardPageViewModel(Application application = null, IApplicationLifetime applicationLifetime = null, Action logoutAction = null)
        {
            _application = application ?? Application.Current;
            _applicationLifetime = applicationLifetime ?? App.Current?.ApplicationLifetime;
            _logoutAction = logoutAction ?? (() =>
            {
                if (_applicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
                {
                    desktop.MainWindow.Content = new HomePageView
                    {
        ...
        private void ToggleTheme()
        {
            ...
            {
                var currentTheme = _application.RequestedThemeVariant;
                _application.RequestedThemeVariant = currentTheme == ThemeVariant.Dark ? ThemeVariant.Light : ThemeVariant.Dark;
            }
        }
    }
}