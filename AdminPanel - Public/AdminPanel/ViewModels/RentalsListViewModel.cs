    ...
    public class RentalsListViewModel : ViewModelBase
    {
            ...
            try
            {
                ...
                if (isSuccess && rentals != null)
                {
                    ...
                    foreach (var rental in rentals)
                    {
                        ...
                        {
                            rental.User = userResult.User;
                        }
                        else
                        {
                            rental.User.Username = $"Nieznany (ID: {rental.UserId})";
                        }
        ...
        private async Task CancelRental(int rentalId)
        {
            try
            {
                ...
                var statusResult = await RentalService.UpdateRentalStatus(rentalId, "Anulowane");
                ...
                {
                    rentalToCancel.RentalStatus = "Anulowane";
                });
        ...
        private async Task ResumeRental(int rentalId)
        {
            try
            {
                ...
                var statusResult = await RentalService.UpdateRentalStatus(rentalId, "Aktywne");
                if (!statusResult.IsSuccess)
                {
                ...
                await Dispatcher.UIThread.InvokeAsync(() =>
                {
                    rentalToResume.RentalStatus = "Aktywne";
                });
